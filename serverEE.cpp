...
int main()
{
    ...
    create_UDP(sockfd, serverAddr, SERVER_EE, LOCAL_IP);

    if (bind(sockfd, (struct sockaddr*)&serverAddr, sizeof(serverAddr))){
        cout << SERVER_EE << " Error in binding." << endl;
		return -1;
    }

    cout << "The ServerEE is up and running using UDP on port " << SERVER_EE << endl;
    ...
    while (true) {
        ...
        auto iter = ee_map.find(courseCode);
        if (iter == ee_map.end()) {
            message = "NOT_FOUND";
